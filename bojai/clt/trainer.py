...
class TrainingManager:
    ...
    def get_required_hyperparams(self, task_type):
        ...
        if task_type == 'clt':
            return ['learning_rate', 'num_epochs', 'num_batches', 'hidden_size', 'num_layers']
        else:
            raise ValueError(f"Unsupported task type: {task_type}")
    
    def edit_hyper_params(self, new_hyperparams : dict):
        if new_hyperparams['num_batches'] == 1:
            raise ValueError('num_batches cannot be 1')
        self.hyperparams = new_hyperparams
        self.trainer = TrainerCLN(self.model, self.training, self.eval, self.device, self.tokenizer, new_hyperparams)
...
class TrainerCLN(Trainer): 
    def __init__(self, model, train_dataset, eval_dataset, device, tokenizer, hyperparams):
        ...
        output_size = len(np.unique(np.array(eval_dataset.outputs)))
        self.model.initialise(hyperparams['hidden_size'], output_size, hyperparams['num_batches'], hyperparams['num_layers'])
        
        self.train_loader = DataLoader(train_dataset, batch_size=self.num_batches, shuffle=True, drop_last=True)
        self.val_loader = DataLoader(eval_dataset, batch_size=self.num_batches, shuffle=False, drop_last=True)
    ...
    def train(self, qthread = None , progress_updated = None, loss_updated = None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for x, y in self.train_loader:
                hidden = hidden.detach()  # Detach hidden state between batches
                x = x.unsqueeze(1).expand(-1, 100, 100)
                x = x.to(torch.float32)
                y = y.to(torch.long)  # Ensure y is of type long (CrossEntropyLoss requirement)
    ...
    def evaluate(self, eval_dataset=None):
        ...
        if eval_dataset is None:
            ...
            val_loader = DataLoader(eval_dataset, batch_size=self.num_batches, shuffle=False)
        ...
        with torch.no_grad():  # Disable gradient calculations during evaluation
            for x, y in val_loader:
                x = x.to(torch.float32)
                x = x.unsqueeze(1).expand(-1, 100, 100)  # Expand dims
                y = y.to(torch.long)  # Ensure correct dtype

                y_predicted, hidden = self.model(x, hidden)  # Get predictions
                pred = torch.argmax(y_predicted, dim=1)
                # No need to unsqueeze
                correct_predictions += pred.eq(y).sum().item()  # Count correct predictions
