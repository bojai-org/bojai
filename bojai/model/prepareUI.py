...
class PrepWindow(QWidget):
    def __init__(self, prep: Prepare, trained = False):
        ...
        # Update Data Section
        sidebar_layout.addLayout(self.create_update_data_section()) #disabled due to problems 
    ...
    def create_info_section(self):
        ...
        tokenizer_type = self.GetGroupBox("Tokenizer Type", str(self.prep.tokenizer.__class__.__name__))
        info_layout.addLayout(tokenizer_type, 1,1)

        ready_prep = self.GetGroupBox("Ready to start training", "yes" if self.prep.prep_ready else "no")
    ...
    def view_tokenized(self):
        ...
        try:
            ...
            message = "the index you entered is too big, make sure you enter a right one"
    ...
    def show_deploy_window(self):
        success_msg = QtWidgets.QMessageBox()
        success_msg.setWindowTitle("Error")
        success_msg.setText("Deployment can only happen after training. Go to training first")
        success_msg.setIcon(QtWidgets.QMessageBox.Information)
        success_msg.setStandardButtons(QtWidgets.QMessageBox.Ok)
        success_msg.exec_()
