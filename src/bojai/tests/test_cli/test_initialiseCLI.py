...
import builtins
import pytest
from initialiseCLI import initialize_pipeline_cli, browseDict, options
...
def patch_core(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "initialiseCLI.prepare_cli", lambda prep: print("📦 PREP CLI CALLED")
    )
    return tmp_path
...
def test_triggers_data_prep(fake_inputs, patch_core, monkeypatch, capsys):
    monkeypatch.setattr(
        "initialiseCLI.prepare_cli", lambda prep: print("📦 DATA PREP TRIGGERED")
    )
    fake_inputs(["GoModel", str(patch_core), "0.6", "0.4", "p"])
...
def test_invalid_split(fake_inputs, patch_core, capsys):
    fake_inputs(["SplitModel", str(patch_core), "0.3", "0.3"])
...
def test_architecture_selection_invalid_index(
    fake_inputs, patch_core, monkeypatch, capsys
):
    monkeypatch.setitem(browseDict, "options", 1)
    monkeypatch.setitem(options, "mock_arch", lambda: "TokenizerInstance")

    fake_inputs(
        ["InvalidArchModel", str(patch_core), "0.5", "0.5", "99"]  # Invalid index
    )
