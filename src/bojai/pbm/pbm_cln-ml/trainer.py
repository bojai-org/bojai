...
class TrainerCLN(Trainer): 
    def __init__(self, model, train_dataset, eval_dataset, device, tokenizer, hyperparams):
        ...
        self.train_loader = DataLoader(train_dataset, batch_size=self.num_batches, shuffle=True)
        self.val_loader = DataLoader(eval_dataset, batch_size=self.num_batches, shuffle=False)
    ...
    def evaluate(self, eval_dataset=None):
        ...
        with torch.no_grad():  # Disable gradient calculations during evaluation
            for x, y in val_loader:
                ...
                predicted_labels = torch.round(outputs)  # Convert probabilities to binary predictions
