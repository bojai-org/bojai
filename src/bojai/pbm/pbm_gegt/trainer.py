    ...
    def train(self, qthread=None, progress_updated=None, loss_updated=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for pixel_vals, labels in self.train_loader:
                ...
                shift_labels = labels[
                    :, 1:
                ].contiguous()  # Remove first token from labels
    ...
    def evaluate(self, eval_dataset=None):
        ...
        with torch.no_grad():  # Disable gradient calculations during evaluation
            for pixel_vals, labels in val_loader:
                ...
                total_tokens += (
                    shift_labels.numel()
                )  # Total number of tokens for perplexity calculation

        # Calculate perplexity
        perplexity = torch.exp(torch.tensor(total_loss / total_tokens))
        if eval_dataset != None: 
            self.logger.log(eval_score=perplexity.item())
