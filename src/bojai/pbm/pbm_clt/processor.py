...
class ProcessorManager():
    ...
    def decide_which_processor(self, task_type):
        if task_type == 'clt': 
            self.processor = ProcessorCLT(self.data_dir, self.division, self.model, self.device, self.tokenizer,512)
            self.train = ProcessorCLT(self.processor.train_dir, [1,0], self.model, self.device, self.tokenizer,512, False)
            self.eval = ProcessorCLT(self.processor.eval_dir, [0,1], self.model, self.device, self.tokenizer,512, False)
...
class ProcessorCLT(Processor, Dataset):
    ...
    def get_inputs_outputs(self):
        ...
        for i in range(len(outputs)):
            if outputs[i] == 2: 
              outputs[i] = 0
        combined = [f"input : {input_text}, output : {output_text}" for input_text, output_text in zip(inputs, outputs)]
