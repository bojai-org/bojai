...
class ProcessorManager():
    def __init__(self, data_dir, division, model, device, tokenizer, task_type, main = True):
    ...
    def decide_which_processor(self, task_type, main):
        if task_type == 'cln':
            self.processor = ProcessorNumber(self.data_dir, self.division, self.model, self.device, self.tokenizer, main)
            if  main:
                self.train = ProcessorNumber(self.processor.train_dir, [1,0], self.model, self.device, self.tokenizer, False)
                self.eval = ProcessorNumber(self.processor.eval_dir, [0,1], self.model, self.device, self.tokenizer, False)
...
class ProcessorNumber(Processor, Dataset):
    ...
    def get_eval_train(self):
        ...
        self.save_to_file(os.path.join('train_dir', "data.json"), self.inputs_train, self.outputs_train)

        # Save evaluation data to text files
        self.save_to_file(os.path.join('eval_dir', "data.json"), self.inputs_eval, self.outputs_eval)

        return os.path.join('train_dir', "data.json"), os.path.join('eval_dir', "data.json")
