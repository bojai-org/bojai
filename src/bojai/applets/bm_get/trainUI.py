...
class TrainWindow(QtWidgets.QWidget):
    def __init__(self, train: Train, deploy=None, trained=False):
        ...
        # Outer vertical layout to wrap everything
        ...
        self.big.addWidget(title)

        # Top Navigation Bar (3 buttons)
        self.bar = QFrame()
        ...
        self.big.addWidget(self.bar)

        # Main content grid layout
        self.main_layout = QGridLayout()

        # Fill grid with sections
        self.main_layout.addLayout(self.create_info_section(), 0, 0)
        self.main_layout.addLayout(self.update_hyperparam_layout(), 0, 1)
        self.main_layout.addLayout(self.create_train_section(), 1, 0)
        self.main_layout.addLayout(self.create_eval_layout(), 2, 0)
        self.main_layout.addLayout(self.replace_model_layout(), 3, 0)
        self.main_layout.addLayout(self.save_load_session_layout(), 1, 1)
        self.main_layout.addLayout(self.visualize_model(), 2, 1)

        # Add the grid layout to the vertical layout
        self.big.addLayout(self.main_layout)
    ...
    def load_session(self):
        ...
        if ok and session_name:
            ...
            self.main_layout.addLayout(info_layout, 0, 0)  # Hyperparameter section below
            msg = QMessageBox()
    ...
    def update_hyperparams(self):
        ...
        self.main_layout.addLayout(info_layout_new, 0, 0)
    ...
    def create_info_section(self):
        ...
        title.setStyleSheet("font-size: 16px; color: black; font-weight: bold;")
        info_layout.addRow(title)

        # Description
        description = QLabel("See the values of the hyperparameters listed below.")
        description.setStyleSheet("font-size: 16px; color: black;")
        info_layout.addRow(description)

        self.hyperparam_view_inputs = []
        ...
        try:
            ...
            for param_name, param_value in self.train.trainerManager.hyperparams.items():
                ...
                self.hyperparam_view_inputs.append(input_field)

                info_layout.addRow(label)
                info_layout.addRow(input_field)

            # Add device
            label = QLabel("device")
            label.setStyleSheet("font-size: 16px; color: black;")

            device_input = QLineEdit(str(self.train.prep.device))
            device_input.setStyleSheet("""
                background-color: #f4eaf9;
                color: black;
                border-radius: 10px;
                padding: 10px;
                font-size: 16px;
            """)
            device_input.setDisabled(True)

            info_layout.addRow(label)
            info_layout.addRow(device_input)
