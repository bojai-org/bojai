...
class TrainingManager:
    ...
    def start_model(self):
        with Timer("Training phase - init - get required hyperparams"):
        ...
        if self.task_type == "cli":
            ...
            with Timer("Training phase - init - TrainerCLN init"):
...
class TrainerCLN(Trainer):
    def __init__(
        ...
        with Timer("Training phase - init - TrainerCLN - super"):
            super().__init__(model, device, tokenizer, hyperparams)
        self.train_dataset = train_dataset
        with Timer("Training phase - init - TrainerCLN - dataloader 1"):
            self.train_loader = DataLoader(
                train_dataset, batch_size=self.num_batches, shuffle=True, drop_last=True
            )
        with Timer("Training phase - init - TrainerCLN - dataloader 2"):
            self.val_loader = DataLoader(
                eval_dataset, batch_size=self.num_batches, shuffle=False, drop_last=True
            )
        with Timer("Training phase - init - TrainerCLN - croosEnt loss"):
            self.loss_fn = nn.CrossEntropyLoss()

        with Timer("Training phase - init - TrainerCLN - Adam"):
    ...
    def train(self, qthread=None, progress_updated=None, loss_updated=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            self.logger.log(epoch=epoch, train=train_score, valid=valid, loss=avg_loss)
